...
- New `st config --auto-watch` / `--no-auto-watch` toggle (default off), shown in `st config`. When enabled, the upcoming surfacing points (status/graph/submit) will self-heal a stale `state.repo` and register the repo with the daemon inline instead of warning.

## 0.9.13

- New command `st get` — adopt the remote version of a stack. When the daemon (or a different session/worktree) restacks and force-pushes your branches, `origin/*` becomes the source of truth and your local checkout is stale. `st get` fetches, then resets each local stack branch to `origin/<branch>` wherever it is safe: when local is strictly behind (fast-forward) or the remote is a clean rebase of the *same* patches (the daemon case — every local-only commit is patch-equivalent to a remote one). Branches with real un-pushed local commits are kept and reported as diverged; `st get --force` discards them and takes the remote. A branch checked out dirty (the current branch or a second worktree) is skipped with a commit-or-stash hint, even under `--force`. It also walks the dependency chain upward and adopts each ancestor stack first (root-down), so adopting a dependent stack pulls in its rewritten parents and keeps the seam ancestor-correct; `--force` applies only to the resolved stack (a diverged branch in an ancestor is reported with `st get -s <ancestor> --force`), and an ancestor mid-restack is skipped with a warning. Snapshots first, so `st undo` reverts a bad adoption. Supports `--dry-run`.
- `st sync` now runs that same adoption (resolved stack plus all its ancestors) as its first step. This fixes the incident where a stale local cut ran `st sync` after the daemon had already restacked + pushed: sync used to rebase the *stale local commits* onto trunk, duplicating or conflicting with the rebase that already landed. Now sync adopts the remote refs first, so a fully-daemon-restacked stack just prints "Nothing to sync" — no duplicate rebase, no conflicts. Branches with genuine local-only work are left alone and flagged (run `st get --force`, or `st get -s <ancestor> --force` for an ancestor, if the remote is the truth).
