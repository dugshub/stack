#!/bin/bash
# Reset todo repo to clean state for Scene 2 (build from scratch)
set -e
...
for branch in $(git branch --list 'dugshub/*' 'backend/*' 'caching/*' 'frontend/*' 'testing/*' | tr -d ' *'); do
  git branch -D "$branch" 2>/dev/null || true
done

# Delete remote branches
for ref in $(git branch -r --list 'origin/dugshub/*' 'origin/backend/*' 'origin/caching/*' 'origin/frontend/*' 'origin/testing/*' | tr -d ' '); do
  branch="${ref#origin/}"
