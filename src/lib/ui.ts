...
export function heading(msg: string): void {
  process.stderr.write(`${pc.bold(msg)}\n`);
}
...
export function stackTree(
  ...
  for (let i = 0; i < stack.branches.length; i++) {
    ...
    const marker = isCurrent ? pc.cyan('\u2190 you are here') : '';
    const nameStr = isCurrent ? pc.bold(branch.name) : branch.name;
...
export function branchTable(rows: BranchRow[]): void {
  ...
  for (const row of rows) {
    ...
    const name = row.isCurrent ? pc.bold(row.name) : row.name;
...
export function positionReport(pos: StackPosition): void {
  info(
    `Branch ${pos.index + 1} of ${pos.total} in stack ${pc.bold(pos.stackName)}`,
  );
}
